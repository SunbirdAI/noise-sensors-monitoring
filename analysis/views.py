...
class ReceiveMetricsFileViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        now = datetime.now()
        metric_files = device.metricstextfile_set.filter(
            time_uploaded__range=[now - timedelta(minutes=30), now]
        )
...
class ListMetricsFilesView(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        result = {"number_of_files": num_files, "metric_files": serializer.data}
    ...
    def get_queryset(self, past_days=1):
        ...
        queryset = queryset.filter(
            time_uploaded__range=[today - timedelta(days=past_days), today]
        )
...
class AggregateMetricsView(APIView):
    def post(self, request):
        ...
        device = Device.objects.get(device_id=device_id)
        metric_files = device.metricstextfile_set.filter(
            time_uploaded__range=[today - end_hours, today - start_hours]
        )
        processed_files = []
        for metric_file in metric_files:
            metrics_data = parse_file(metric_file.metrics_file.file, device_id)
...
def get_analysis_queryset(api_view_object: ListAPIView, hourly=True):
    ...
    queryset = queryset.filter(
        date__range=[datetime.now() - timedelta(days=past_days), datetime.now()]
    )
